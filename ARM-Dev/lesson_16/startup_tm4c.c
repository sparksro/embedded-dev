...
extern int CSTACK$$Limit; // keyword extern tells the compiler its not a new int but one linked in from an external source.
void __iar_program_start(void); //  this is a function so it needs to be declared like one.
void Unused_Handler(void);

int const __vector_table[] @ ".intvec" = { //_@ ".intvec"  is special functionality provided by IAR to make sure this goes into the correct part of memory.  There is no standard C functionallity to do this.
  (int)&CSTACK$$Limit,//stack pointer
  (int)&__iar_program_start,  //this is an address of the function for Reset
  //next set from tm4c_cmis Interrupt Handler Prototypes
  (int)&NMI_Handler,
  ...
  0, //reserved
  0, //reserved
  //The following are not faults so they get re-assigned bellow
  (int)&SVC_Handler,
...
__stackless void HardFault_Handler(void) {
  //standard way to do this is with a whil(1) loop but this is a pain as then its stuck in an endless loop.
  assert_failed("HardFault", __LINE__);// this reports the error and records it in an error log and resets the machine.
}
__stackless void NMI_Handler(void) {
  assert_failed("NMI", __LINE__);
}
__stackless void MemManage_Handler(void) {
  assert_failed("MemFault", __LINE__);
}
...
// the following are used if they were not used allready -thus week handlers
