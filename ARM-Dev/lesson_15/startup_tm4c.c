...
extern int CSTACK$$Limit; // keyword kextern tells the compiler its not a new int but one linked in from an external source.
...
int const __vector_table[] @ ".intvec" = { //_@ ".intvec"  is special functionality provided by IAR to make sure this goes into the correct part of memory.  There is no standard C functionallity to do this.
  ...
  0, //reserved
  0, //reserved
  //The following are not faults so they get reasigned bellow
  (int)&SVC_Handler,
